...
class CourseInfoService:
    """
    Making an API to retrieve the necessary data for retrieving course details
    for each course ID acquired from the CUB catalog.
    """
    total_courses = 0
        ...
        for course_id in course_list:
            try:
                resp = requests.get(FetchCoursesConstants.COURSE_INFO_URL, timeout=30,
                                    data=json.dumps(cls._get_course_info_body(course_id)))
                if resp.status_code == 200:
                    for course in resp.json()["results"]:
                        course_info_list.append(cls._reterive_necessary_info(course))
                cls.total_courses -= 1
    ...
    @classmethod
    def _get_course_info_body(cls, course_id: str) -> dict:
        return {
            "other": {
                "srcdb": cls.srcDB
            },
    ...
    @classmethod
    def _reterive_necessary_info(cls, course_info: dict) -> dict:
        return {
